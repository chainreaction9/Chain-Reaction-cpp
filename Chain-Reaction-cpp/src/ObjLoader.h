/**
* This file is a by-product of the GitHub project <https://github.com/Bly7/OBJ-Loader> owned by Robert Smith 
* and is distributed under the MIT License. The current file contains minor modifications of the original source code.
*/
...
namespace objl{
	...
	public:
		...
		/**
		* Load a .obj file into the loader.
		* If file is loaded returns true.
		* If the file is unable to be found,
		* or unable to be loaded returns false.
		...
		*/  
			...
			while (std::getline(file, curline)){
				...
				if (algorithm::firstToken(curline) == "usemtl")
				{
					...
					if (!Indices.empty() && !Vertices.empty())
					{
						...
						tempMesh.MeshName = meshname + "_" + std::to_string(index+1);
		...
		/**
		* Newly added feature by author <Shubhamoy Nandan>.
		* Loads `.obj` data from byte stream.
		* Does not support .mtl data processing. However, it assigns a default (plastic) material property to each MeshEntry.
		* @param rawData: pointer to the byte stream containing .obj data.
		...
		*/
		bool LoadFromMemory(const unsigned char* rawData, size_t size) {
			...
			std::string curline;
			while (std::getline(rawStream, curline)) {
				// Generate a Mesh Object or Prepare for an object to be created
				if (algorithm::firstToken(curline) == "o" || algorithm::firstToken(curline) == "g" || curline[0] == 'g') {
					if (!listening) {
						listening = true;
			...
			// Set a default Material for each Mesh
			for (unsigned int i = 0; i < MeshMatNames.size(); i++) {
				std::string matname = MeshMatNames[i];
				if (!(matname.compare(""))) matname.assign("default");
				//Set a default (plastic) material property.
				LoadedMeshes[i].MeshMaterial.name = matname;
				LoadedMeshes[i].MeshMaterial.illum = 2;
				LoadedMeshes[i].MeshMaterial.Kd = objl::Vector3(1.0f, 1.0f, 1.0f);
				LoadedMeshes[i].MeshMaterial.Ka = objl::Vector3(1.0f, 1.0f, 1.0f);
				LoadedMeshes[i].MeshMaterial.Ns = 233.333333f;
				LoadedMeshes[i].MeshMaterial.d = 1.0f;
				LoadedMeshes[i].MeshMaterial.Ni = 1.0f;
				LoadedMeshes[i].MeshMaterial.Ks = objl::Vector3(1.0f, 1.0f, 1.0f);
			}
			if (LoadedMeshes.empty() && LoadedVertices.empty() && LoadedIndices.empty()) {
				return false;
			}
		...
		/**
		* Newly added feature by author <Shubhamoy Nandan>.
		* Loads `.mtl` data from byte stream.
		* @param rawData: pointer to the byte stream containing .obj data.
		* @param size: size of the obj data in byte length.
			...
			while (std::getline(rawStream, curline)){
				...
				if (algorithm::firstToken(curline) == "newmtl"){
					if (!listening){
						...
						materialProcessed++;
						storageLocation[tempMaterial.name] = tempMaterial;
	...
	private:
		...
		*/
		...
		bool LoadMaterials(std::string path){
			...
			// Push Back loaded Material
			if (tempMaterial.name.compare("")) LoadedMaterials.push_back(tempMaterial);
